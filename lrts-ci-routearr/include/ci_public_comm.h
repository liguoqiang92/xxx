...
/*******************************************************************************
 * 定义 宏macro、常量const、枚举量enum
 *
 ******************************************************************************/
#define OCCU        1U                   /* 占用 */
#define CLEAR       2U                   /* 出清 */
#define BLOCKADE    1U                   /* 封锁 */
#define UNBLOCKADE  2U                   /* 解封 */
#define LOCK        1U                   /* 锁闭 */
#define UNLOCK      2U                   /* 解锁 */

#define TRACK_NUM   32                   /* 轨道数量 */
#define ROUTE_NUM   14                   /* 进路数量 */
#define SIGNAL_NUM  15                   /* 信号机数量 */
#define BALISE_NUM 99
#define BALI_MESS_SIZE 4
...
typedef struct _route{
    ...
    uint16_t r_status;                 /* 进路状态 占用=1,出清=0 */
    ...
    uint16_t *r_t_auto_list_2;         /* 自动触发进路区段列表2 from ATS：
                                          没有为None，有为ID列表 */
    uint16_t r_t_protect;              /* 保护区段：没有为None，有为ID */
 ...
 *
 ******************************************************************************/
uint8_t main_loop();                  /* 主循环 */
uint8_t main_logic();                 /* 主逻辑 */
uint8_t data_init();                  /* 工程数据初始化 */
uint16_t stub_fun(msg_t m, void *p);
